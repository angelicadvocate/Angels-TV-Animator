 ...
 *   new ATAIntegration('myAnimationElement');
 *   
 *   // With custom options
 *   new ATAIntegration('myElement', {
...
class ATAIntegration {
    ...
    createStatusIndicator() {
        ...
        this.statusIndicator.className = 'ata-status-indicator disconnected';
        this.statusIndicator.id = 'ataStatusIndicator';
        this.statusIndicator.title = 'ATA Connection Status';
    ...
    showRefreshNotification(data) {
        ...
        notification.className = 'ata-notification';
    ...
    flashAnimation() {
        ...
        this.mainElement.classList.remove('ata-flash-animation');
        // Force reflow
        this.mainElement.offsetHeight;
        this.mainElement.classList.add('ata-flash-animation');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof io === 'undefined') {
        console.warn('ATA Integration: Socket.IO not found. Please include Socket.IO library before ata-integration.js');
        return;
    }
    
    // Auto-initialize only if no manual initialization detected
    setTimeout(() => {
        if (!window.ataIntegration) {
            console.log('Auto-initializing ATA Integration...');
            window.ataIntegration = new ATAIntegration();
        }
    }, 100);
});
...
setTimeout(() => {
    if (typeof io !== 'undefined' && (!window.ataIntegration || !window.ataIntegration.isConnected())) {
...
window.ATAIntegration = ATAIntegration;